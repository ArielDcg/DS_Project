...
void runAlgorithm(Grid &grid, MazeAlgorithm &algo, int cellSize, const std::string &title, const sf::Font *fontPtr = nullptr) {
    ...
    while (window.isOpen()) {
        ...
        if (fontPtr) {
            ...
            if (solverChosen) {
                std::string stats;
                if (solverType == SOLVER_DFS && dfsSolver) {
                    stats = "DFS: nodos=" + std::to_string(dfsSolver->getNodesExplored()) +
                            " tiempo=" + std::to_string(dfsSolver->getElapsedMs()) + " ms";
                } else if (solverType == SOLVER_ASTAR && aStarSolver) {
                    stats = "A*: nodos=" + std::to_string(aStarSolver->getNodesExplored()) +
                            " tiempo=" + std::to_string(aStarSolver->getElapsedMs()) + " ms";
                } else if (solverType == SOLVER_GREEDY && greedySolver) {
                    stats = "Greedy: nodos=" + std::to_string(greedySolver->getNodesExplored()) +
                            " tiempo=" + std::to_string(greedySolver->getElapsedMs()) + " ms";
                } else if (solverType == SOLVER_UCS && ucsSolver) {
                    stats = "UCS: nodos=" + std::to_string(ucsSolver->getNodesExplored()) +
                            " tiempo=" + std::to_string(ucsSolver->getElapsedMs()) + " ms";
                }

                sf::Text statsText(*fontPtr, stats, 14);
                statsText.setPosition(sf::Vector2f(6.f, 60.f)); // debajo del título
                statsText.setFillColor(sf::Color(255, 255, 180, 230));
