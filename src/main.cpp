...
// --- Factories para crear cada solver ---
std::unique_ptr<MazeSolver> makeDFSSolver(Grid &grid, Coord s, Coord g) {
    return std::make_unique<MazeSolver>(grid, s, g);
}
std::unique_ptr<AStarSolver> makeAStarSolver(Grid &grid, Coord s, Coord g) {
    return std::make_unique<AStarSolver>(grid, s, g);
}
std::unique_ptr<GreedySolver> makeGreedySolver(Grid &grid, Coord s, Coord g) {
    return std::make_unique<GreedySolver>(grid, s, g);
}
std::unique_ptr<UCSSolver> makeUCSSolver(Grid &grid, Coord s, Coord g) {
    return std::make_unique<UCSSolver>(grid, s, g);
}

// --- Función principal de benchmark ---
void runBenchmarkSingle(Grid &gridTemplate) {
    RankingManager ranking;

    Coord start(0,0);
    Coord goal(gridTemplate.width()-1, gridTemplate.height()-1);

    auto runSolverGeneric = [&](const std::string &name, auto makeSolver) -> SolverMetrics {
        Grid grid = gridTemplate; // copia del laberinto

        auto solver = makeSolver(grid, start, goal);
        ...
        SolverMetrics m;
        m.name = name;
        m.nodesExplored = solver->getNodesExplored();
        m.pathLength    = solver->getSolution().size();
        m.timeMs        = ms;

        return m;
    };

    ranking.add(runSolverGeneric("DFS",    makeDFSSolver));
    ranking.add(runSolverGeneric("A*",     makeAStarSolver));
    ranking.add(runSolverGeneric("Greedy", makeGreedySolver));
    ranking.add(runSolverGeneric("UCS",    makeUCSSolver));
...
int main(int argc, char** argv) {
        // === modo benchmark por línea de comandos ===
    if (argc > 1 && std::string(argv[1]) == "--benchmark") {
        const int W = 40, H = 28;
        Grid grid(W, H);

        // genera un laberinto consistente para todos
        DFSAlgorithm genAlgo(grid);
        while (!genAlgo.finished()) genAlgo.step();

        runBenchmarkSingle(grid);
        return 0;
    }
